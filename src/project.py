...
"""
my notes:
- this CLI manages camera presets (JSON), exports my Blender add-on file to the repo root,
  and can generate a simple thumbnail PNG (Pillow).
- keep commands small and clear.

Run examples I can paste into terminal (from the repo root):
...
# try Pillow; if not installed, only the thumbnail command will warn
try:
...
# compute important paths once; make sure data/ exists
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DATA_DIR = os.path.join(ROOT, "data")
os.makedirs(DATA_DIR, exist_ok=True)
PRESETS_PATH = os.path.join(DATA_DIR, "camera_presets.json")

# default add-on source in src/, export copy goes to repo root (easy Blender install)
...
# load all presets from JSON; be defensive if file is missing/bad
    ...
    try:
        ...
        for row in raw:
            try:
                ...
                # skip rows that don't match our fields
                continue
        return out
    except Exception as e:
        # FIXED: proper f-string (no parentheses call)
        print(f"[WARN] Could not read {PRESETS_PATH}: {e}")
        return []


# save all presets to JSON (pretty formatting)
...
# simple title PNG with letterbox bars; works with default font if Arial missing
...
# list all presets or suggest 'add' if none exist
@cli.command("list")
def cmd_list():
    presets = load_all()
    if not presets:
    ...
    for p in presets:
        print(
            f"- {p.name}: aspect={p.aspect}, focal={p.focal_mm}mm, "
            f"sensor={p.sensor_mm}, shutter={p.shutter_deg}°, f/{p.fstop}"
        )


# add a preset with basic validation (sensor format) and unique name
@cli.command("add")
...
# remove a preset by name; if not found, just say so
@cli.command("remove")
@click.option("--name", required=True)
def cmd_remove(name):
    presets = load_all()
...
def cmd_export_addon(src_path, out_path):
    # make paths absolute so it works no matter where I run from
    src_abs = os.path.abspath(src_path)
    out_abs = os.path.abspath(out_path)
    try:
        if not os.path.exists(src_abs):
            print(f"[WARN] Source add-on not found at: {src_abs}")
            # show a quick listing of the expected folder to help me debug a typo/path issue
            try:
                print(f"[INFO] Files in {os.path.dirname(src_abs)}: {os.listdir(os.path.dirname(src_abs))}")
            except Exception as e:
                print(f"[INFO] Could not list directory: {e}")
            sys.exit(1)
        with open(src_abs, "r", encoding="utf-8") as f_in:
            code = f_in.read()
        with open(out_abs, "w", encoding="utf-8") as f_out:
            f_out.write(code)
        print(f"[OK] Exported add-on → {out_abs}")
    except Exception as e:
        print(f"[ERROR] Could not export add-on: {e}")
        sys.exit(1)


# optional thumbnail (tests Pillow + simple image ops)
